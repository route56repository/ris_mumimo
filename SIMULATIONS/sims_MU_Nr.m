...
Nr = 1000;                                 % Nº of elements per column
...
% OPtotimize RIS phases to maximize alpha
%[fases_rsm,it_rsm] = RIS_oPtot_RSM(random_phases, Hbu_og, matHbr, matHru,[f1,f2,f3,f4]);
[fases_zf,it_zf] = RIS_opt_ZF(random_phases, Hbu_og, matHbr, matHru);
% Compund channel
Hc_zf = (matHru*fases_zf*matHbr); % RSM oPtotimization
Hc_no = (matHru*fases_no*matHbr);   % No oPtotimization
Hbu = Hbu_og*beta_bu;
% Complete channel
...
T = Nris(s)+1;
BERe_bu4(s,:) = BER_4QAM_error(alpha_ZF(Hbu),Ptot,var2,B_bu,Nris(s),T,4,1e-3);
BERe_t4(s,:) = BER_4QAM_error(alpha_ZF(Ht_zf),Ptot,var2,B,Nris(s),T,4,1e-3);

BERe_bu16(s,:) = BER_4QAM_error(alpha_ZF(Hbu),Ptot,var2,B_bu,Nris(s),T,16);
BERe_t16(s,:) = BER_4QAM_error(alpha_ZF(Ht_zf),Ptot,var2,B,Nris(s),T,16);
